...
def glitch(bot, update):
    ...
    if update.message.reply_to_message is not None:
        if "/glitch" in update.message.text:
            try:
                if "http:" in update.message.reply_to_message.text:
                    ...
                    bot.getFile(update.message.reply_to_message.photo[-1].file_id).download(glitch_folder+"original.jpg")
                    process_img(bot, update)
            except:
        ...
        bot.getFile(update.message.photo[-1].file_id).download(glitch_folder+"original.jpg")
        process_img(bot, update)
