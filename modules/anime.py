...
def anime(bot, update, args):
    ...
    try:
        ...
        with open(anime_folder+"anime_temp.jpg","rb") as f:
            bot.sendPhoto(update.message.chat_id, f, caption="Nothing found, onii-chan, but here's one random pic:\n"+cap, reply_to_message_id=update.message.message_id)
        print(datetime.datetime.now(), ">>>", "Tag not found:", input_query, "sent random", ">>>", update.message.from_user.username)
